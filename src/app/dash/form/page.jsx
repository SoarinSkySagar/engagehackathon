...
import React, { useState } from 'react';
import Image from 'next/image';
import Loading from './Loading'; // Make sure to adjust the path to your Loading component
import form from 'public/form.png';
...
const HealthProblemForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false); // Set loading to false once the API response is received
    }
  };
    ...
    if (apiResponse && Array.isArray(apiResponse)) {
      return apiResponse.map((item, index) => (
        <div key={index} className=" items-center mb-2">
          ...
          <div className="relative w-full bg-blue-200 h-6">
            <div
              className="absolute top-0 left-0 h-6 bg-blue-600"
              style={{ width: `${item[0].score * 100}%` }}
            ></div>
          </div>
          <div className="w-60 font-semibold">{item[1].label}:</div>
          <div className="relative w-full bg-blue-200 h-6">
            <div
              className="absolute top-0 left-0 h-6 bg-blue-600"
              style={{ width: `${item[1].score * 100}%` }}
            ></div>
          </div>
          <div className="w-60 font-semibold">{item[2].label}:</div>
          <div className="relative w-full bg-blue-200 h-6">
            <div
              className="absolute top-0 left-0 h-6 bg-blue-600"
              style={{ width: `${item[2].score * 100}%` }}
            ></div>
          </div>
          <div className="w-60 font-semibold">{item[3].label}:</div>
          <div className="relative w-full bg-blue-200 h-6">
            <div
              className="absolute top-0 left-0 h-6 bg-blue-600"
              style={{ width: `${item[3].score * 100}%` }}
            ></div>
          </div>
          <div className="w-60 font-semibold">{item[4].label}:</div>
          <div className="relative w-full bg-blue-200 h-6">
            <div
              className="absolute top-0 left-0 h-6 bg-blue-600"
              style={{ width: `${item[4].score * 100}%` }}
            ></div>
          </div>
        </div>
      ));
      ...
      <div className="flex">
        <div className="mx-20 w-full max-w-md p-3 rounded-lg shadow-md">
          ...
          >
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? <Loading /> : 'Predict'} {/* Show the Loading component during loading */}
            </button>
          </form>
        ...
        <div className="flex-1">
          <Image
            src={form}
            alt=""
            className="ml-[120px] w-[900px] mt-[-10px] h-[800px] object-contain"
          />
        </div>
      </div>
    </div>
